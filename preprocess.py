...
def remove_background(img, bg_thresh):
    ...
    cv2.imwrite("./image.png",foreground)
    foreground_image_cv2 = cv2.imread("./image.png",cv2.IMREAD_GRAYSCALE)
    num_label, labels, status, centroids = cv2.connectedComponentsWithStats(foreground_image_cv2,connectivity=4)
    ...
    if num_label >= 2:
        ...
        for label in range(1, num_label):
            area = status[label, cv2.CC_STAT_AREA]
            if area > max_area:
                max_area = area
                max_label = label

        result_image = np.zeros_like(foreground_image_cv2)
        result_image[labels == max_label] = 1

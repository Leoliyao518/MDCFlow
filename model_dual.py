...
class FeatureExtractor(nn.Module):
    ...
    def forward(self, x):
        ...
        # print(features)
        feature1_2 = torch.cat([down_feature0,features[0],features[1]],dim=1)
        down_feature1_2 = F.interpolate(feature1_2,(24,24),mode="bilinear")
        feature = torch.cat([down_feature1_2,features[2]],dim=1)
        return feature
...
class FeatureProjectionConv(nn.Module):
    def __init__(self, inchannel=1152, outchannel=608):
        ...
        # # 先尝试直接输入放大之后的rgb特征
        self.rconv1 = nn.Conv2d(608, 512, kernel_size=3, padding=1)
        self.rconv2 = nn.Conv2d(512, 256, kernel_size=5, padding=2)
        self.rconv3 = nn.Conv2d(256, 608, kernel_size=3, padding=1)

        # self.rupconv1 = nn.ConvTranspose2d(256, 128, kernel_size=2, stride=2)
        # self.rupconv2 = nn.ConvTranspose2d(256, 512, kernel_size=2, stride=2)
        # self.rupconv3 = nn.ConvTranspose2d(512, 608, kernel_size=2, stride=2)
        # self.rconv5 = nn.Conv2d(608, 608, kernel_size=3, padding=1)
    ...
    def forward(self, x, rgb):
        ...
        # out_rgb = self.relu(self.rconv3(self.relu(self.rconv2(self.relu(self.rconv1(rgb))))))
        #
        return out
        # return out, out_rgb
...
class Model(nn.Module):
    def __init__(self, nf=True, n_blocks=c.n_coupling_blocks, channels_hidden=c.channels_hidden_teacher, depth = False):
        ...
        if nf:
            if depth:
                self.net = get_nf(input_dim=c.n_feat_depth)
            else:
                self.net = get_nf(input_dim=c.n_feat)
        else:
            self.net = Student(channels_hidden=channels_hidden, n_blocks=n_blocks, depth = depth)

...
def cal_feature_map_entropy(feature_map):
    flat_feature_map = feature_map.view(-1)
    hist = torch.histc(flat_feature_map,bins=128,min=feature_map.min().item(),max=feature_map.max().item())
    prob = hist / hist.sum()
...
def train(train_loader, test_loader, scale):
    ...
    for epoch in range(c.epochs):
        ...
        if ((epoch+1) % 4 == 0):
            ...
            au_gt_masks = np.concatenate(au_gt_masks)
            au_score_maps = np.concatenate(au_score_maps)
            ...
            _, aupro30_score = aupro30.update(calculate_au_pro(au_gt_masks, au_score_maps, integration_limit=0.3), epoch, print_score=c.verbose)

            pixel_flag, pixel_score = pixel_obs.update(roc_auc_score(gt_masks, score_maps), epoch,
                                                       print_score=c.verbose)


    return max_nll_obs, pixel_obs, aupro30
